...
export const QuickActionDisplay: React.FC<QuickActionDisplayProps> = ({ type, data, conversationId }) => {
  ...
  if (type === "mcq") {
    ...
    return (
      <div className="mt-4 space-y-4 animate-in fade-in slide-in-from-top-2 duration-300">
        <div className="bg-pink-50 dark:bg-pink-900/20 border border-pink-100 dark:border-pink-800 rounded-xl p-4">
          <div className="space-y-4">
            {mcqQuestions.map((q: any, displayIndex: number) => {
              ...
              return (
                <div key={originalIndex} className="bg-white dark:bg-gray-800 rounded-lg p-3 shadow-sm border border-pink-100 dark:border-pink-800">
                  <div className="flex items-start justify-between mb-2">
                    <div className="flex text-sm font-medium text-gray-800 dark:text-gray-200">
                      <span className="text-pink-500 mr-2">{displayIndex + 1}.</span>
                      <ReactMarkdown
                    ...
                    {showResults && (
                      <div className="ml-2">
                        {isCorrect ? (
                          <i className="fi fi-rr-check-circle text-green-500 text-sm flex items-center justify-center"></i>
                        ) : isIncorrect ? (
                          <i className="fi fi-rr-cross-circle text-red-500 text-sm flex items-center justify-center"></i>
                        ) : (
                          <i className="fi fi-rr-minus-circle text-gray-400 text-sm flex items-center justify-center"></i>
                        )}
                      </div>
                    )}
                  </div>
                  ...
                  <div className="space-y-2 ml-4">
                    {optionsArray.map((opt: string, j: number) => {
                      ...
                      if (showResults) {
                        if (isCorrectOption) {
                          className += "bg-green-50 border-green-200 text-green-700 dark:bg-green-900 dark:border-green-800 dark:text-green-300 font-medium";
                        } else if (isSelected && !isCorrectOption) {
                          className += "bg-red-50 border-red-200 text-red-700 dark:bg-red-900 dark:border-red-800 dark:text-red-300";
                        } else {
                          className += "bg-gray-50 border-gray-100 text-gray-600 dark:bg-gray-700 dark:border-gray-600 dark:text-gray-400";
                        }
                      } else {
                        if (isSelected) {
                          className += "bg-blue-50 border-blue-300 text-blue-700 dark:bg-blue-900 dark:border-blue-600 dark:text-blue-300 font-medium";
                        } else {
                          className += "bg-gray-50 border-gray-200 text-gray-600 dark:bg-gray-700 dark:border-gray-600 dark:text-gray-400 hover:bg-blue-50 hover:border-blue-200 dark:hover:bg-blue-900";
                        }
                      }
                        ...
                        >
                          <div className="flex items-center justify-between">
                            ...
                            {showResults && isCorrectOption && (
                              <i className="fi fi-rr-check ml-2 flex items-center justify-center"></i>
                            )}
                            {showResults && isSelected && !isCorrectOption && (
                              <i className="fi fi-rr-cross ml-2 flex items-center justify-center"></i>
                            )}
            ...
            {!showResults && mcqQuestions.length > 0 && (
              <div className="mt-4 flex gap-2">
                <button
                  ...
                  className="px-4 py-2 bg-gradient-to-br from-logoBlue to-logoViolet hover:shadow-md hover:shadow-logoViolet disabled:bg-gray-300 disabled:cursor-not-allowed text-white text-sm rounded-lg transition-colors font-medium flex items-center gap-2"
                >
                  {isSubmitting && <i className="fi fi-rr-spinner animate-spin flex items-center justify-center"></i>}
  ...
  if (type === "quiz") {
    ...
    return (
      <div className="mt-4 space-y-4 animate-in fade-in slide-in-from-top-2 duration-300">
        <div className="bg-sky-50 border border-sky-100 dark:border-sky-800 rounded-xl p-4">
          <div className="space-y-4">
            {shortAnswerQuestions.map((q: any, displayIndex: number) => {
              ...
              return (
                <div key={originalIndex} className="bg-white dark:bg-gray-800 rounded-lg p-3 shadow-sm border border-indigo-100 dark:border-indigo-800/50">
                  <div className="flex items-start justify-between mb-2">
                    <div className="flex text-sm font-medium text-gray-800 dark:text-gray-200">
                      <span className="text-sky-500 mr-2">{displayIndex + 1}.</span>
                      <ReactMarkdown
                        remarkPlugins={[remarkGfm, remarkMath]}
                        rehypePlugins={[rehypeKatex]}
                      >
                  ...
                  <div className="ml-4 mt-2 space-y-2">
                    <input
                      ...
                      className={`w-full text-sm p-2 rounded border transition-colors ${hasUserAnswer
                        ...
                        : 'bg-white border-sky-200 text-gray-800 dark:bg-gray-700 dark:border-sky-600 dark:text-gray-200 focus:border-sky-400 focus:ring-2 focus:ring-sky-200 dark:focus:ring-sky-800'
                        } outline-none`}
                    />
    ...
    return (
      <div className="">
        {quizData.follow_on_concept && (
          <div className="bg-purple-50 border border-purple-100 dark:border-purple-800 rounded-xl p-3 flex items-start gap-3">
            <i className="fi fi-rr-bulb text-purple-500 mt-0.5"></i>
            <div>
              <p className="text-xs font-semibold text-purple-800 dark:text-purple-200 mb-1">Follow-on Concept</p>
              <p className="text-xs text-purple-700 dark:text-purple-300">{quizData.follow_on_concept}</p>
            </div>
          </div>
        )}
      </div>
